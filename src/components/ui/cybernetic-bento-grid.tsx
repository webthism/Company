...
const BentoItem = ({ className, children, icon: Icon, title, description }: { className?: string, children?: React.ReactNode, icon?: any, title?: string, description?: string }) => {
    ...
    return (
        <div 
            ref={itemRef} 
            className={cn(
                "group relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-white/[0.02] p-8 transition-all duration-500 hover:border-[#BD9DFF]/30",
        ...
        >
            <div className="relative z-40 flex h-full flex-col">
                <div className="mb-6 h-12 w-12 rounded-xl bg-white/5 flex items-center justify-center text-[#BD9DFF] group-hover:bg-[#BD9DFF] group-hover:text-black transition-all duration-500">
                    <Icon size={24} strokeWidth={1} />
                </div>
                <h3 className="text-2xl font-heading font-black tracking-tighter text-white group-hover:text-[#BD9DFF] transition-colors">
                    {title}
                </h3>
                <p className="mt-4 text-[#A1A1AA] font-sans font-extralight text-sm leading-relaxed group-hover:text-white transition-colors">
                    {description}
                </p>
                {children}
            </div>
...
export const CyberneticBentoGrid = () => {
    return (
        <div className="w-full">
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 auto-rows-[18rem]">
                ...
                >
                    <div className="mt-auto pt-10 grid grid-cols-2 gap-4">
                        <div className="h-24 bg-white/5 rounded-2xl border border-white/5 flex items-center justify-center text-[10px] text-[#A1A1AA] uppercase tracking-widest font-bold">
                            Review Pulse
                        </div>
                        <div className="h-24 bg-white/5 rounded-2xl border border-white/5 flex items-center justify-center text-[10px] text-[#A1A1AA] uppercase tracking-widest font-bold">
                            Record Sync
